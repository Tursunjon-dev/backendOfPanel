...
async function main() {
  ...
  // NOTE: Do NOT register a global CORS middleware before /admin.
  // Otherwise preflight OPTIONS for PUT/DELETE will be handled too early
  // (and only allow GET/HEAD), causing "Method PUT is not allowed".

  // Admin: strict CORS (front-end panel)
  app.use(
    '/admin',
    cors({
      origin: (origin, cb) => {
  ...
  // Public: allow GET from anywhere (mobile app, kiosk, etc.)
